...
function TopicsBar() {
    ...
    return (
        <div className="hidden lg:flex flex-col pl-2 items-start w-max mt-2 fixed left-5">
            {topics.map((item) =>
                <h2 key={item} onClick={() => setTopic(item)} className="flex justify-start w-full my-[0.3rem] text-xs rounded-sm bg-gray-900 p-2 hover:bg-gray-800 hover:cursor-pointer hover:text-[#F2BEA0]">{item}</h2>)
            }
