...
function EditProfileModal() {
    ...
    async function onSubmit() {
        ...
        try {
            ...
            toast.error('Could not update the details'); setLoading(false);
        } finally {
            const res = await dispatch(login(userDetails));
            resetDetails(); setLoading(false);
    ...
    return (
        <dialog id="profileModal" className="modal">
            <section className="h-[90vh] lg:w-[30vw] md:w-[80vw] flex flex-col items-center pt-6 justify-center">
                <div className="modal-box w-full bg-gray-900 rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                    <form method="dialog">
                    ...
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <div className="space-y-4 md:space-y-6">
                            ...
                            <button onClick={onSubmit} id="submitButton" className="w-full text-white bg-gray-700 py-2 rounded-md hover:bg-gray-800 transition-all ease-in-out">{loading ? 'Updating your account ...' : 'Update account'}</button>
                        </div>
                    </div>
                </div>
            </section>
