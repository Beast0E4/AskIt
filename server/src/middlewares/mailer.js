...
const url = "https://res.cloudinary.com/dnaznkzoy/image/upload/v1724169395/Untitled_design-removebg-preview_osnjam.png"
const webpage = process.env.FRONT_URL
...
const sendWelcomeEmail = async (email) => {
  try {
    const mailOptions = {
      from: `"AskIt" <${EMAIL}>`,
      to: email,
      subject: "Welcome to AskIt!",
      text: "Welcome to AskIt! We're excited to have you on board. Start exploring and connecting with friends today!",
      html: `
        ...
        <div style="width: 100%; text-align: center; margin-bottom: 15px;">
          <table style="margin: 0 auto;">
              <tr>
                  ...
                  <td>
                      <h2 style="color: #023e8a; margin: 0; white-space: nowrap;">Welcome to AskIt!</h2>
                  </td>
              </tr>
          </table>
      </div>
...
const forgetPasswordLink = async (email,token) => {
  try {
    const mailOptions = {
      from: `"AskIt" <${EMAIL}>`,
      to: email,
      subject: "Reset Your Password - AskIt",
      ...
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: auto; border: 1px solid #ddd; padding: 20px; border-radius: 10px; box-shadow: 2px 2px 10px rgba(0,0,0,0.1); text-align: center; background-color: #f9f9f9;">
        ...
        <p style="font-size: 16px; color: #555;">We received a request to reset your AskIt password.</p>
...
const passwordChangedEmail = async (email) => {
  try {
    const mailOptions = {
      from: `"AskIt" <${EMAIL}>`,
      to: email,
      subject: "Your Password Has Been Successfully Changed - AskIt",
      text: `Hello,
